...
const Game = () => {
  ...
  const { finishGame, duzzle, setDuzzle, gameState } = useGameContext();
    ...
    <>
      ...
      <div className="flex w-full justify-center gap-2 px-4 lg:mb-40">
        <input
          ...
          aria-label="answer"
          aria-labelledby="write your answer"
          autoFocus
          ref={textFieldRef}
          onKeyDown={handleKeyDown}
          placeholder={duzzle[current].placeholder}
        />
        <PrimaryButton onPress={() => saveAnswer()} aria-labelledby="answer">
          <ReplyIcon />
        </PrimaryButton>
        <SecondaryButton onPress={passLetter} aria-labelledby="pass">
          <NextIcon />
        </SecondaryButton>
      </div>
    </>
